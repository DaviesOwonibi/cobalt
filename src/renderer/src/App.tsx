...
function App(): JSX.Element {
	...
	// Setttings State Management
	const [settingsVisibility, setSettingsVisibility] = useState(false);
	const settingsBtnRef = useRef<HTMLButtonElement>(null);
	const settingsMenuRef = useRef<HTMLDivElement>(null);

	// Find Functionality State Management
	...
	// Helper Functions
	const createNewTab = (name: string, url: string): Tab => ({
		name,
		url,
		id: idCounter,
		key: `tab-${idCounter}`
	...
	const updateTab = (id: number, updates: Partial<Tab>): void => {
		setTabs((prevTabs) =>
	...
	function parseLocaleString(time: Date): string {
		...
		const year = time.getFullYear(); // <-- Add this line
		...
		return `${formattedDay} ${month} ${year} ${timeString}`; // <-- Include year here
	}
	...
	function addTab(): void {
		const newTab = createNewTab('New Tab', '');
	...
	async function openLinkInNewTab(url: string): Promise<void> {
		const newTab: Tab = { name: '', id: idCounter, url: url, key: `tab-${idCounter}` };
		setTabs([...tabs, newTab]);
	...
	const handleAddTheme = (e: React.FormEvent<HTMLFormElement>): void => {
		...
		if (newThemeName !== '') {
			if (themeNames.includes(newThemeName)) {
				...
				popup.success({
					title: 'Success',
					message: `${newThemeName} has been added.`
				});
	...
	async function handleSearchSubmit(event: React.FormEvent<HTMLFormElement>): Promise<void> {
		...
		if (searchInput) {
			...
			if (httpsPattern.test(searchInput)) {
				...
				const rest = lowercaseName.slice(1);
				const newName = firstLetter.toUpperCase() + rest;

				// Use updateTab to update the tab's name and URL
				updateTab(activeTab, { url: newUrl, name: newName });
				return;
