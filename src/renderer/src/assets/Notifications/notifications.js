...
export default function Notification(options = {}) {
	...
	const setProperty = (obj = {}) => {
		...
		opts =
			!!obj && obj.constructor.name === 'Object' ? Object.assign({}, defOpts, obj) : defOpts;
		// check position
	...
	const hidePopUp = (elPopup) => {
		...
		elPopup.classList.remove(
			elPopup.dataset.position === 'center' ? animationFadeInClass : animationInClass
		);
		elPopup.classList.add(
			elPopup.dataset.position === 'center' ? animationFadeOutClass : animationOutClass
		);
